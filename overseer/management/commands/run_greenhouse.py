...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        while True:
                ...
                for username, boxname in running_machines:
                    ...
                    if VagrantBox.objects.filter(username=username, boxname=boxname).filter(Q(status_code="F")|Q(status_code="D")).count():
                        ...
                        logger.warning("VagrantBox {}/{} was found to be running in global status but NOT in DB. Destroying.".format(username, boxname),
                                       extra={'box': '{}/{}'.format(username, boxname)}
                                       )
                ...
                for vagBox_tmp1 in VagrantBox.objects.filter(Q(status_code="R")|Q(status_code="I")).filter(worker_name=os.uname()[1]):
                    if [vagBox_tmp1.username, vagBox_tmp1.boxname] not in running_machines:
                        logger.warning("VagrantBox {}/{} was found NOT to be running in global status but is DB. Marking as failed.".format(vagBox_tmp1.username, vagBox_tmp1.boxname),
                                       extra={'box': '{}/{}'.format(vagBox_tmp1.username, vagBox_tmp1.boxname)})
            ...
            # add to waiting pool if less than MAXPOOL
            ...
            if VagrantBox.objects.filter(status_code='W').count() < MAX_POOL:
                #vagBox_obj = VagrantBox.objects.filter(processed_at__isnull=True).exclude(status_code="W").first()
                logger.info("Waiting list '{}' < '{}' POOL, requesting new boxes".format(VagrantBox.objects.filter(status_code='W').count(), MAX_POOL))
                addMoreToQueue()
            ...
            if psutil.virtual_memory().available > MEMORY_FREELIMIT and VagrantBox.objects.filter(status_code='W'):
                logger.debug("Enough free memory '{}' and enough '{}' boxes waiting, proceeding to POOL condition".format(
                    psutil.virtual_memory().available // 1024 // 1024, VagrantBox.objects.filter(status_code='W').count()
                ))

                if VagrantBox.objects.filter(worker_name=os.uname()[1]).filter(Q(status_code='R') | Q(status_code='I')).count() < MAX_POOL:
                    ...
                    logger.info(
                        "There are already '{}' I/R machines running on host and POOL is '{}', skipping to sleep".format(
                            VagrantBox.objects.filter(worker_name=os.uname()[1]).filter(Q(status_code='R') | Q(status_code='I')).count() ,
                            MAX_POOL
                        ))

            if psutil.virtual_memory().available < MEMORY_FREELIMIT:
                logger.warning("Not enough memory ({}MB) to run another Boxes, going to sleep before loop".format(
                        psutil.virtual_memory().available // 1024 // 1024))

            if not VagrantBox.objects.filter(status_code='W').count() and psutil.virtual_memory().available > MEMORY_FREELIMIT:
